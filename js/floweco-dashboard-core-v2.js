...
(function() {
    ...
    async function loadSummary() {
        try {
            ...
            let budgetUsed = 0;
            budgets.forEach(b => {
                const catSpent = categoryTotals[b.category] || 0;
                budgetUsed += Math.min(catSpent, parseFloat(b.amount || 0));
            });
            ...
            const budgetUsedEl = document.getElementById('dashBudgetUsed');
            const budgetBarFillEl = document.getElementById('dashBudgetBarFill');
            ...
            if (budgetUsedEl) {
                if (totalBudget > 0) {
                    budgetUsedEl.innerHTML = `${formatCurrency(totalExpenses)}<span style="font-size: 0.7rem; color: #9CA3AF;"> / ${formatCurrency(totalBudget)}</span>`;
                } else {
                    budgetUsedEl.textContent = 'לא הוגדר';
                }
            }
            
            if (budgetBarFillEl) {
                const cappedPercent = Math.min(budgetPercent, 100);
                budgetBarFillEl.style.width = cappedPercent + '%';
                
                // Color based on usage
                budgetBarFillEl.classList.remove('warning', 'danger');
                if (budgetPercent >= 90) {
                    budgetBarFillEl.classList.add('danger');
                } else if (budgetPercent >= 75) {
                    budgetBarFillEl.classList.add('warning');
                }
            }
